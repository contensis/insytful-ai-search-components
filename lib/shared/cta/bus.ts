 ...
 */
export function getInsytfulAISearchEvents(): EventTarget | null {
  if (typeof window === "undefined") return null;
  const existing = window.insytfulAISearchEvents;
  if (existing instanceof EventTarget && !(existing instanceof Node)) {
    return existing;
  }
  ...
  return (window.insytfulAISearchEvents = new EventTarget());
}
